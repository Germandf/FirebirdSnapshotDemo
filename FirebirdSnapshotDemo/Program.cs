...
var isolationCode = await snapshotCtx.Database.SqlQueryRaw<int>(
    ...
    .SingleAsync();
Console.WriteLine($"[SNAPSHOT] Began transaction with Isolation code: {isolationCode} (1 = SNAPSHOT)");

var before = await snapshotCtx.TestEntities.CountAsync();
Console.WriteLine($"[SNAPSHOT] Count: {before}");
...
await snapshotTx.DisposeAsync();
Console.WriteLine("[SNAPSHOT] Transaction disposed");

var after = await snapshotCtx.TestEntities.CountAsync();
Console.WriteLine($"[SNAPSHOT] Count after transaction disposed: {after}");
